...
class Claim:
    ...
    @classmethod
    def create(cls, issuer: Party, holder: Party, amount: Money) -> "Claim":
...
class ClaimBook:
    """Minimal domain model for bilateral financial claims."""
    ...
    def issue(self, issuer: Party, holder: Party, amount: Money) -> Claim:
    ...
    def transfer(self, claim_id: UUID, *, from_holder: Party, to_holder: Party) -> Claim:
        ...
        if to_holder.id == claim.issuer.id:
            raise ClaimError("Transfer to issuer is settlement; use settle().")
    ...
    def settle(self, claim_id: UUID, *, holder: Party) -> Claim:
        claim = self._require_claim(claim_id)
        if claim.holder.id != holder.id:
            raise ClaimError("Only the current holder can settle the claim.")
        del self._claims[claim.id]
        self._assert_balanced()
        return claim

    def assets_of(self, party: Party) -> Money:
        currency = self._currency()
        total = sum((c.amount.amount for c in self._claims.values() if c.holder.id == party.id), start=0)
        return Money.of(total, currency)

    def liabilities_of(self, party: Party) -> Money:
        currency = self._currency()
        total = sum((c.amount.amount for c in self._claims.values() if c.issuer.id == party.id), start=0)
        return Money.of(total, currency)
    ...
    def _require_currency_compatible(self, amount: Money) -> None:
        if self._currency_code is not None and amount.currency != self._currency_code:
            raise ClaimError("Chapter 01 supports one currency at a time.")
